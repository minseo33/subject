package com.intern.backend.backend.endpoint.api.board.support;

import lombok.Getter;

@Getter // 필드값을 가져오기위해서
public class Pagination {

	// 1. 페이지 당 보여지는 게시글의 최대 개수
	private int pageSize = 10;

	// 2. 페이징된 버튼의 블럭당 최대 개수
	private int blockSize = 5;

	// 3. 현재 페이지
	private int page = 1;

	// 4. 현재 블럭
	private int block = 1;

	// 5. 총 게시글 수
	private int totalListCnt;

	// 6. 총 페이지 수
	private int totalPageCnt;

	// 7. 총 블럭 수 //blockSize로 인해 생성된 블럭의 총 개수 -> blocksize값이 5이면 15/5 =3
	private int totalBlockCnt;

	// 8. 블럭 시작 페이지
	private int startPage = 1;

	// 9. 블럭 마지막 페이지
	private int endPage = 1;

	// 10. DB 접근 시작 index //db에서 select할 때, 어디서부터 ~ 어디까지
	private int startIndex = 0;

	// 11. 이전 블럭의 마지막 페이지
	private int prevBlock;

	// 12. 다음 블럭의 시작 페이지
	private int nextBlock;

	
	// 전체개수, 현재 페이지, 페이지당 표출할 개수를 매개변수로 받음
	public Pagination(int totalListCnt, int page, int pageSize) {

		// 총 게시물 수(totalListCnt)와 현재 페이지(page), 페이지당 표출할 개수(pageSize)를
		// BoardController / BoardApiController 로 부터 전달받음.

//      setPageSize(pageSize);
		this.pageSize = pageSize;

		// 3. 현재 페이지
//      setPage(page);
		this.page = page;

		// 5. 총 게시글 수
//      setTotalListCnt(totalListCnt);
		this.totalListCnt = totalListCnt;

		// 6. 총 페이지 수
		// 한 페이지의 최대 개수를 총 게시물 수 * 1.0로 나누어주고 올림 해준다. // 10/(150*1.0) = 15
		// 총 페이지 수
//      setTotalPageCnt((int) Math.ceil(totalListCnt * 1.0 / pageSize));
		this.totalPageCnt = (int) Math.ceil(totalListCnt * 1.0 / pageSize);

		// 7. 총 블럭 수
		// 한 블럭의 최대 개수를 총 페이지의 수 * 1.0로 나누어주고 올림 해준다.
		// 총 블럭 수
//      setTotalBlockCnt((int) Math.ceil(totalPageCnt * 1.0 / blockSize));
		this.totalBlockCnt = (int) Math.ceil(totalPageCnt * 1.0 / blockSize);

		// 4. 현재 블럭
		// 현재 페이지 * 1.0을 블록의 최대 개수로 나누어주고 올림한다.
		// 현재 블록
//      setBlock((int) Math.ceil((page * 1.0)/blockSize));
		this.block = (int) Math.ceil((page * 1.0) / blockSize);

		// 8. 블럭 시작 페이지
//      setStartPage((block - 1) * blockSize + 1); => (1-1) * 1+1 =>0
		this.startPage = (block - 1) * blockSize + 1;
		System.out.println(startPage);

		// 9. 블럭 마지막 페이지
//      setEndPage(startPage + blockSize - 1);
		this.endPage = startPage + blockSize - 1;

		// 블럭 마지막 페이지에 대한 validation //마지막 > 총페이지 -> 쉽게 마지막페이지에서 다음을 눌러도 마지막
		if (endPage > totalPageCnt) {
			this.endPage = totalPageCnt;
		}

		// 10. DB 접근 시작 index //1-1*10 = 0
//    setStartIndex((page-1) * pageSize);
		this.startIndex = (page - 1) * pageSize;

		// 11. 이전 블럭(클릭 시, 이전 블럭 마지막 페이지)
//      setPrevBlock((block * blockSize) - blockSize);
		this.prevBlock = (block * blockSize) - blockSize;

		// 이전 블럭에 대한 validation //1보다 작은값 이전 눌러도 1페이지
		if (prevBlock < 1) {
			this.prevBlock = 1;
		}

		// 12. 다음 블럭(클릭 시, 다음 블럭 첫번째 페이지)
//      setNextBlock((block * blockSize) + 1);
		this.nextBlock = (block * blockSize) + 1;

		// 다음 블럭에 대한 validation
		if (nextBlock > totalPageCnt) {
			nextBlock = totalPageCnt;
		}

		// pagination의 필드값들 변경됨
	}
}